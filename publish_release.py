...
VERSION = "v1.2.1"
RELEASE_TITLE = "FakeGPS Pro v1.2.1 - 🍄 蘑菇戰情純 Python 原生加解密（免裝 Node.js）與極速秒回"

RELEASE_BODY = """# FakeGPS Pro v1.2.1 - 🍄 蘑菇戰情純 Python 原生架構與穩定性重大升級

本版本徹底解決舊版對外部執行環境的依賴，帶來 100% 綠色解壓即用的極致體驗與毫秒級查詢速度：

## 🌟 重點更新內容

### 1. 🚀 徹底根除 `[WinError 2]`，蘑菇雷達改為純 Python 原生加解密（免裝 Node.js）
* **零環境依賴**：逆向重構皮皮蘑菇安全通訊協定，採用純 Python 原生實現動態密鑰握手、**AES-256-CBC (PKCS#7)** 加密與 **HMAC-SHA256** 數位簽名。
* **徹底免裝 Node.js**：舊版因調用外部 `node.exe` 導致未安裝 Node.js 的電腦出現 `[WinError 2]`，現在完全解壓即用、100% 獨立運行！
* **查詢耗時暴降**：省去喚醒外部進程與 JS 虛擬機開銷，查詢耗時由 2~3 秒縮短至 **0.5 秒內即時秒回**！

### 2. 🍄 蘑菇全尺寸等級 (Level) 與屬性 (Type) 自訂追蹤
...
* **Windows 使用者**：下載 `FakeGPS_Pro_Windows_v1.2.1.zip`，解壓縮後執行 `FakeGPS_Pro.exe` 即可（亦可直接在軟體內點「🔄 檢查更新」一鍵線上自動升級！）。
